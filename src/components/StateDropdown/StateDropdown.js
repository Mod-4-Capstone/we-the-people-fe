...
const StateDropdown = (props) => {
  ...
  const createStateOptions = () => {
    return states.map((state, i) => {
      return (
        <option value={state.abbrev} key={i}>
          {state.fullName}
        </option>
      );
    });
  ...
  return (
    <div className="state-dropdown-container">
      <p className="state-dropdown-description">
        See Representatives by State:
      </p>
      <form className="state-form-container" onSubmit={(e) => handleSubmit(e)}>
        ...
        <Popup
          trigger={
            <span>
              <button className="go-button" disabled={!props.selectedState}>
                Go!
              </button>
            </span>
          }
